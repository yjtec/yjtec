...
const renderFooter = ({ tabList, tabActiveKey, onTabChange, tabBarExtraContent, }) => {
    if (tabList && tabList.length) {
        return (<Tabs className={`${prefixedClassName}-tabs`} activeKey={tabActiveKey} onChange={key => {
            if (onTabChange) {
                onTabChange(key);
            }
...
const renderPageHeader = (content, extraContent) => {
    ...
    return (<div className={`${prefixedClassName}-detail`}>
      <div className={`${prefixedClassName}-main`}>
        <div className={`${prefixedClassName}-row`}>
          ...
          {extraContent && (<div className={`${prefixedClassName}-extraContent`}>
              {extraContent}
            </div>)}
        </div>
      </div>
...
const defaultPageHeaderRender = (props) => {
    ...
    return (<RouteContext.Consumer>
      {value => {
        ...
        return (<PageHeader {...value} title={pageHeaderTitle} {...restProps} footer={renderFooter(restProps)}>
